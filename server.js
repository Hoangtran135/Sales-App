...
app.delete("/deleteusers/:id", (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE id = ?", [id], (err, user) => {
    ...
    if (user.length === 0) {
      ...
      return res
        .status(404)
        .json({ message: "Không tìm thấy tài khoản để ẩn" });
    }
    ...
    db.query(updateQuery, [id], (err, result) => {
      ...
      if (result.affectedRows === 0) {
        ...
        return res
          .status(404)
          .json({ message: "Không thể ẩn tài khoản này" });
      }
...
app.get("/users", (req, res) => {
  ...
  const query = "SELECT id, username, email, role FROM users WHERE role = ? AND role1 = ?";
